    ...
    public function penalizeDriver(Request $request, Response $response)
    {
        ...
        try {
            ...
            if (!$deducted) {
                ...
                if (!$stmt->fetch()) {
                    $db->prepare("INSERT INTO wallets (user_id, balance) VALUES (?, 0)")->execute([$driverId]);
                }
                $db->prepare("UPDATE wallets SET balance = balance - ? WHERE user_id = ?")->execute([$penaltyAmount, $driverId]);
                $db->prepare("INSERT INTO wallet_transactions (user_id, amount, type, description, created_at) VALUES (?, ?, 'penalty', ?, NOW())")->execute([$driverId, -$penaltyAmount, $reason]);
            }
